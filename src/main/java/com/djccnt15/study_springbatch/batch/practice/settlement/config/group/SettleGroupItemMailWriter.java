...
import com.djccnt15.study_springbatch.batch.practice.settlement.utils.EmailProvider;
...
public class SettleGroupItemMailWriter implements ItemWriter<List<SettleGroupEntity>> {
    ...
    public void write(Chunk<? extends List<SettleGroupEntity>> chunk) throws Exception {
        for (List<SettleGroupEntity> settleGroups : chunk) {
            ...
            var settleGroup = settleGroups.get(0);
            var customerId = settleGroup.getCustomerId();
            var customer = customerRepository.findById(customerId);
            
            var totalCount = settleGroups.stream().map(SettleGroupEntity::getTotalCount).reduce(0L, Long::sum);
            var totalFee = settleGroups.stream().map(SettleGroupEntity::getTotalFee).reduce(0L, Long::sum);
            
            var detailByService = settleGroups.stream()
